...
/**
  ...
  * The output integers should be in the same sequence as they were in the input. You need to write a function with the
  * recommended method signature for the languages mentioned below. For the rest of the languages,
  ...
  * We recommend not using the inbuilt library function.
  *
  * Sample Input
  *
  * 3
  ...
  * Explanation
  *
  * 2, 1, and 0 are the list elements that are less than the X delimiter, 3. They are displayed in the same order
...
object FilterArray {
  ...
  def fn(delim: Int, arr: List[Int]): List[Int] = arr match {
    case Nil => Nil
    case head :: tail => {
      if (head < delim) head :: fn(delim, tail)
      else fn(delim, tail)
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val result1 = f1(delim, arr)
    println(result1.mkString("\n"))

    println("------------------------")

    val result2 = fn(delim, arr)
